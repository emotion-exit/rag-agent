...
from app.services.auth import get_current_user, get_request_user
...
def _get_request_scoped_user(request: Request) -> dict:
    user = get_request_user(request) or get_current_user(required=True)
    return user


@router.post("/save-jobs")
async def submit_note_save_job(payload: NoteSaveRequest, request: Request):
    user = _get_request_scoped_user(request)
...
async def get_note_job_status(job_id: str, request: Request):
    user = _get_request_scoped_user(request)
    job = get_note_save_job(user_id=str(user.get("user_id", "") or "").strip(), job_id=job_id)
    if job is None:
...
@router.get("")
async def get_note_list(request: Request):
    user = _get_request_scoped_user(request)
    return {
        "items": list_notes(user_id=str(user.get("user_id", "") or "").strip())
    }
...
async def get_note(note_id: str, request: Request):
    user = _get_request_scoped_user(request)
    detail = get_note_detail(user_id=str(user.get("user_id", "") or "").strip(), note_id=note_id)
    if detail is None:
...
async def submit_note_revision_save_job(
    note_id: str,
