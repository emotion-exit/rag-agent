...
flowchart TD
    ...
    G --> H[归一化 CHROMA_PERSIST_DIR / UPLOAD_DIR]
...
```mermaid
flowchart TD
    A[KnowledgeBaseView 选择文件并上传] --> B[POST /api/knowledge-base/upload]
    B --> C[校验扩展名 / 文件大小 / 空文件]
    ...
    M --> N[返回 chunks_created / image_count]
```
...
```mermaid
flowchart TD
    A[ChatView 输入问题] --> B[POST /api/chat/stream]
