...
当前项目里的“流数据”不是简单的字符串拼接，而是基于 SSE 的结构化事件流。

后端统一以 `text/event-stream` 返回，单个事件的基本结构是：
...
前端没有直接使用 EventSource，而是采用 `fetch + ReadableStream + TextDecoder` 手动消费。这样可以继续使用 POST 请求体提交 `message`、`session_id` 和 `retrieval_filters`，同时保留认证头和公开配置头。

从当前实现看，流数据主要分成 5 类，每一类的处理目标都不同。
...
- `start` 负责让前端立即进入加载态，提示“已接收问题，正在准备检索”
- `done` 负责关闭本轮流，前端把消息状态切成 `done`，并清空进度提示
- `error` 负责显式结束异常路径，前端会把消息状态置为 `error`，并展示失败原因

这类事件不承载正文，只负责驱动一次流请求的生命周期。
...
- `progress` 和兼容分支 `retrieval` 会进入 `progressSteps`，用于展示检索和生成阶段的步骤提示
- `retrieval_meta` 主要承载 query variants，不直接渲染成正文，而是作为检索侧元信息存到消息对象
- `scope` 用于把当前轮识别出的 `knowledge_space` 回填到前端状态，后续追问或澄清重试可以直接复用

这类事件的原则是“只更新状态，不写入答案正文”。
...
- 后端在知识库无法唯一收敛，或召回结果存在多义时，直接发出 `clarify`
- 前端收到后，把说明文本写入当前 assistant 消息，并挂上 `options`
- 同时把消息直接标记为 `done`，停止继续等待答案流
- 用户点击澄清选项后，再把选中的字段回写到 `retrievalFilters`，并基于原问题重新发起请求

这一类事件本质上不是内容流，而是“流式过程中插入的人机确认”。
...
- `thought` 承载可展示的中间说明，但后端已经做过清洗，不直接暴露原始思维链
- `answer` 是最终答案主通道，前端会持续追加到 `answerContent`
- `text` 是兜底类型，当前主要用于兼容非结构化返回；前端会按答案片段处理，避免有输出却不显示

这一类事件统一采用“增量追加”策略，而不是整段覆盖：

- 前端把新增片段累积到消息对象里，形成逐段渲染
- 后端会比较已发送文本，只下发尚未发送的新增部分，避免重复内容
...
- `sources` 不和正文混发，而是在答案真正开始输出时单独发送
- 前端收到后直接替换 `assistantMsg.sources`
- 用户点击来源卡片后，再调用来源详情接口拉取 excerpt 和关联图片

这类事件的重点是把“正文”和“证据”拆开处理，避免正文流中夹杂大量结构化来源数据。

#### 6. 后端处理流数据的关键策略

后端并不是把模型原始输出直接透传给前端，而是先做一层事件整形：

- 先检索、后生成，证据不足时直接走拒答分支
- 在第一次进入答案阶段时补发 `progress`，提示已经从检索切换到生成
- 在答案开始输出时插入 `sources`，让来源和答案自然对齐
- 对模型输出做清洗，剥离思维链标签、内部函数名、接口路径和占位模板
- 即使上游一次输出大段文本，也会拆成小块并加轻微延迟，维持更稳定的流式观感

因此这里的后端职责不是“转发 token”，而是“把上游输出重组为用户可消费的事件语义”。

#### 7. 前端消费流数据的关键策略

前端的关键不是联网本身，而是“拆帧、分派、兜底”：

1. 用 `response.body.getReader()` 持续读取字节块。
2. 用 `TextDecoder.decode(value, { stream: true })` 做增量解码。
3. 把换行统一成 `\n`。
4. 以 `\n\n` 为边界拆出完整 SSE 事件块。
5. 提取 `data:` 行并拼接成 payload。
6. 优先按 JSON 解析，再根据 `type` 分发到不同处理分支。

这里有两个关键兜底：

- 使用 `buffer` 保存半包，防止事件跨 chunk 被截断
- 流结束后再解析一次尾部残留内容，避免最后一个事件丢失

#### 8. 可以抽象出的通用做法

当前项目适合沉淀出的通用结论是：

先把流数据按“控制、进度、交互、内容、来源”分层，再由后端负责事件整形，前端只负责增量消费和状态落位。

这样做的好处是：

- 新增事件类型时，不容易破坏既有正文渲染逻辑
- 每类事件职责清晰，排障成本更低
- 特别适合 RAG 场景，因为检索状态、澄清动作和来源证据都不是普通文本，不能和答案正文混成一类
