...
- Web / 开发态：前端通过 runtime.ts 读取 VITE_API_BASE 或默认地址，直接请求后端
- 桌面态：Electron 主进程拉起本地 Python 后端，并把前端作为桌面应用窗口承载

在这两种模式下，业务 API 保持一致，差别主要在配置来源和后端启动方式。
...
桌面模式下，Electron 主进程负责：

- 在用户目录创建 config.json
- 用默认值补全缺失字段
- 对温度、chunk 参数、检索参数做归一化
- 在保存时仅允许白名单字段写入
- 保存成功后自动重启内置后端

后端启动入口是 backend/run_desktop.py，本质上仍然是通过 uvicorn 启动 FastAPI，只是 host、port、日志级别改为从 Electron 注入的环境变量读取。
...
```mermaid
flowchart TD
  A["Electron 或 Web 前端启动"] --> B["runtime.ts 解析运行模式"]
  B --> C{"是否桌面态"}
  C -->|是| D["Electron ensureDesktopConfig 创建或读取 config.json"]
  D --> E["注入 APP_CONFIG_PATH BACKEND_HOST BACKEND_PORT"]
  E --> F["run_desktop.py 启动 FastAPI"]
  C -->|否| G["前端按 VITE_API_BASE 直接请求后端"]
  F --> H["config.py 合并 JSON / 环境变量 / 默认值"]
  G --> H
  H --> I["归一化本地目录与请求头配置"]
  I --> J["main.py 创建数据目录并挂载路由"]
...
sequenceDiagram
    ...
    User->>FE: 打开设置页
    FE->>Runtime: 判断当前是 Desktop 还是 Web

    alt Desktop
      FE->>Main: getConfig()
      Main-->>FE: 返回本地 config.json 合并结果
      FE->>API: GET /health
    ...
    User->>FE: 修改并保存
    alt Desktop
      FE->>Main: saveConfig(form)
      Main->>Main: 写入 config.json 并重启后端
      Main-->>FE: 返回保存后的配置
    else Web
...
当前知识库不是单层目录，而是树形知识空间结构。空间数据保存在 upload_dir 下的 knowledge_spaces.json，并在后端被整理成：
...
这意味着文档上传并不是简单地写入向量库，而是先绑定到某个知识空间，再继承该空间的语义上下文。
...
5. 根据目标知识空间补全 semantic metadata
...
真正写入索引的语义字段由后端根据 knowledge space 自动推导：

- knowledge_space 取空间 path
- category 取空间 category
- topic 取空间 topic
- tags 取空间 tags 与上传 tags 合并结果
- version_label 优先使用上传值，否则回退到空间默认值
...
系统支持把未归类文档迁移到正式知识空间，而且迁移分为同步和异步两种接口。

迁移时不会只改 metadata，而是会：

1. 读取原有 chunk
2. 用目标空间 metadata 重新生成 embedding 内容
3. 删除旧索引
4. 按新空间重新写入 Chroma

这样迁移后的语义表示和目标空间保持一致，检索结果不会因为旧前缀残留而失真。

### 8. 入库流程图

```mermaid
flowchart TD
  A["KnowledgeBaseView 选择知识空间和文件"] --> B["POST /api/knowledge-base/upload-jobs"]
...
```mermaid
flowchart TD
  A["发起未归类迁移"] --> B["POST /api/knowledge-base/documents/migrate-ungrouped/jobs"]
  B --> C["create_job 生成迁移任务"]
  C --> D["后台线程 _run_migration_job"]
  D --> E["读取未归类文档与已存储 chunks"]
  E --> F["使用目标空间 metadata 重新生成 embedding 内容"]
  F --> G["delete_document 删除旧索引"]
  G --> H["add_documents 重建新索引"]
  H --> I["advance_job 更新迁移进度"]
...
1. 根据显式 filters 或问题文本判断知识空间
2. 如有必要，用 LLM 做知识空间辅助判定
...
- chunk_index
- knowledge_space
- category、topic、tags、version_label
